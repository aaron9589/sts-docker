# REST API Implementation Analysis
...
1. ❌ Used `station_id` instead of `station` in locations table joins
2. ❌ Used non-existent `destination` field in car_orders (should be `shipment`)
3. ❌ Reposition logic may have issues with shipment vs destination confusion
4. ❌ History record location field usage unclear
