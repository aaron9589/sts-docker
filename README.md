...
## Getting Started - Pulling the published release
...
5) Edit the `docker-compose.yml` file under the `web_prebuilt` section where the comments are located to suit your use case. Read the comments alongside each setting!
6) Run `docker compose up --profile prebuild; up` and wait for the containers to start
...
Steps 1-5 as above, and run `docker compose up --profile build; up --build` to build and run a local instance of the container.